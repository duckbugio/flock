...
| Inbound audio / video | Direct MP3/MP4 sources are exposed by newer LO releases; player-only catalog entries may omit `file_path` | Guarded, bounded download into the chat workspace; the agent receives the file and caption. Missing bytes produce a per-file notice and no run |
